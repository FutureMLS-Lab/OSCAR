...
def main() -> None:
    ...
    if any(len(block) != block_size for block in blocks):
        raise ValueError("KL blocks do not match the cached block size")
    if not 0 < args.positions_per_block < block_size:
        raise ValueError("KL positions must be smaller than a block")
    ...
    if args.teacher_reference is None:
        ...
        output = {
            ...
            "protocol": (
                f"wikitext2_{block_size}_tail"
                f"{args.positions_per_block}_top{args.top_k}_v1"
            ),
            "model": args.model,
        ...
        if teacher.get("protocol") != expected_protocol:
