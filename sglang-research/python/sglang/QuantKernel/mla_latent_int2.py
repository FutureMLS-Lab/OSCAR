...
def _scatter_pack_kernel(
    ...
    for g in tl.static_range(NG):
        ...
        packed = tl.zeros([nb], dtype=tl.int32)
        for j in tl.static_range(4):
            xj = tl.load(x_ptr + pid * D + g * GS + 4 * ob + j).to(tl.float32)
            if LLOYD:
                zj = tl.fdiv(xj - mean, std, ieee_rounding=True)
                qj = ((zj >= T0).to(tl.float32)
                      + (zj >= T1).to(tl.float32)
                ...
                qj = tl.minimum(tl.maximum(qj, 0.0), 3.0)
            packed |= qj.to(tl.int32) << (2 * j)
        tl.store(codes_ptr + s * (D // 4) + g * nb + ob, packed.to(tl.uint8))
...
def _gather_dequant_kernel(
    ...
    offs = tl.arange(0, D)
    gid = offs // GS
    byte = tl.load(codes_ptr + s * (D // 4) + offs // 4).to(tl.int32)
    q = ((byte >> (2 * (offs % 4))) & 0x3).to(tl.float32)
...
def scatter_pack_rows(x, slots, codes_buf, params_buf, group_size, lloyd_max):
    ...
    _scatter_pack_kernel[(n,)](
        ...
        D=d, GS=group_size, NG=d // group_size, LLOYD=lloyd_max,
        T0=_LM_THRESHOLDS[0], T1=_LM_THRESHOLDS[1], T2=_LM_THRESHOLDS[2],
        LM_SPAN3=_LM_SPAN / 3.0, LM_RATIO=_LM_RATIO, LM_C0=_LM_CENTROIDS[0],
    )


def gather_dequant_rows(slots, codes_buf, params_buf, out, group_size, lloyd_max):
