"""
CoQuant-style per-row quantile clip + int2 KV cache pack kernels.

This module hosts the Triton kernels used when
``SGLANG_COQUANT_ROTATION_MODE == "coquant"``. Rotation itself is done as a bf16
...
def _vectorized_elems_per_thread(dtype: torch.dtype) -> int:
    ...
    the multi-row clip kernel is only used in the coquant path where the HP
...
#   * K and V share head_dim (true for MHA configs targeted by coquant mixed-KV)
#
...
@triton.jit
def _kv_coquant_rotate_k_clip_single_kernel(
    k_input_ptr,
    v_input_ptr,
    R_ptr,
    loc_ptr,
...
def quantized_set_kv_int2_coquant_rotate_k_clip_triton(
    ...
    """Single-launch fused coquant K-rotation + clip + quantize + int2 pack for
    ...
    assert cache_k_unrotated.shape == cache_v_rotated.shape, (
        "K/V shape mismatch in coquant_rotate_k_clip (kernel requires identical "
    ...
    assert _is_power_of_two(head_dim), (
        f"coquant rotate+clip int2 kernel requires power-of-two head_dim, got {head_dim}"
    )
    ...
    if _get_num_scale_groups(k_scales_zeros_buffer) != 1:
        raise NotImplementedError(
            "coquant rotate+clip+quant fused kernel requires single-scale K layout "
            f"(got num_groups={_get_num_scale_groups(k_scales_zeros_buffer)})"
        )
    if _get_num_scale_groups(v_scales_zeros_buffer) != 1:
        raise NotImplementedError(
            "coquant rotate+clip+quant fused kernel requires single-scale V layout "
    ...
    grid = (triton.cdiv(num_tokens, block_tok), num_heads)
    _kv_coquant_rotate_k_clip_single_kernel[grid](
        cache_k_unrotated,
        cache_v_rotated,
        R_k,
        loc,
