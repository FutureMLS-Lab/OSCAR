...
class Scheduler(
    ...
    def init_moe_gemm_config(self):
        ...
        if hasattr(config_to_check, "num_experts_per_tok") or hasattr(
            config_to_check, "num_experts_per_token"
        ):
    ...
    def maybe_wait_mixed_kv_forward_done(self):
        ...
        forward_done = getattr(result, "forward_done", None)
        if forward_done is None:
            return

        # Stash the event on the kv_pool. The wait is issued later, inside
        # ``_alloc_for_decode_mixed`` at the apply-quant-and-remap boundary,
        # so host-syncing pre-flush work (radix eviction, retract free,
        # plan-kernel free) does not stall behind the previous forward. See
        # plan-for-a-fix-starry-russell.md.
        kv_pool = self.token_to_kv_pool_allocator.get_kvcache()
        stash = getattr(kv_pool, "stash_pending_forward", None)
    ...
    def _build_decode_batch_from_reqs(self, reqs):
        """Build a decode-ready ScheduleBatch for requests with committed KV."""
        device = self.device
    ...
    def _promote_completed_chunked_req_to_running(self, req: Req) -> bool:
        ...
        batch = self._build_decode_batch_from_reqs([req])
        if self.running_batch.is_empty():
    ...
    def _get_new_batch_prefill_raw(
        ...
        if self.chunked_req is not None:
            ...
            if chunked_req.extend_input_len <= 0:
            ...
            if self.enable_overlap:
                ...
                with self.forward_stream_ctx, self.record_bubble_metrics(batch):
                    ...
                    if self.enable_mixed_kv_stream_wait:
                        ...
                        batch_result.forward_done.record()
