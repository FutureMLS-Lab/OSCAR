...
def _situ_fn(input, beta, linear_beta):
    """SituGLU over an INTERLEAVED gate/up row: g0, u0, g1, u1, ...

    Reshaping to (rows, n, 2) and splitting the trailing pair is what makes
    this the interleaved reading. Kimi-K3 does not use this one; see
    _situ_fn_halves.
    """
    gate, up = tl.split(
...
@triton.jit
def _situ_fn_halves(input, beta, linear_beta):
    """SituGLU over a NON-INTERLEAVED gate/up row: [gate half | up half].

    Kimi-K3's expert weights land this way -- the FusedMoE loader writes w1
    (gate_proj) at offset 0 and w3 (up_proj) at offset shard_size, which is
    what gate_up_interleaved=False on its FusedMoE means. Reading it with the
    interleaved kernel pairs gate[i] with gate[i+1] for every expert, which
    does not fail, it just returns the wrong numbers: K3 served fluent-looking
    token soup for as long as this variant was missing.

    Reshape to (rows, 2, n) puts the two halves on the middle axis; transpose
    brings the pair to the trailing axis so tl.split can take it.
    """
    # Shapes must stay inline: binding input.shape[1] // 2 to a name loses its
    # constexpr type and the kernel fails to compile with "Shape element 1
    # must have type constexpr[int]".
    gate, up = tl.split(
        tl.trans(
            tl.reshape(input, (input.shape[0], 2, input.shape[1] // 2)), 0, 2, 1
        )
    )
    gate = gate.to(tl.float32)
    up = up.to(tl.float32)
...
def triton_kernel_fused_experts_with_bias(
    ...
    if activation == "situ":
        act_name = "situ"
        # The layout is not a preference, it is what the loader wrote: with
        # gate_up_interleaved=False, w13 is [gate | up] halves, not alternating
        # pairs. Picking the wrong reader is silent.
        act_fn = _situ_fn if gate_up_interleaved else _situ_fn_halves
