    ...
    def forward_extend(
        ...
        **kwargs,
        ...
        core_attn_out = self.kernel_dispatcher.extend(
            ...
            cache_indices=cache_indices,
            query_start_loc=query_start_loc,
            # Same three the decode path already forwards. `a` now arrives as
            # the RAW forget gate on this path too, so the kernel activates it
            # and takes the cumsum in log2 space itself.
            A_log=layer.A_log,
            dt_bias=layer.dt_bias,
            lower_bound=getattr(layer, "gate_lower_bound", None),
