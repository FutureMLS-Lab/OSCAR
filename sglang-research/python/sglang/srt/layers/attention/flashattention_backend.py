...
class FlashAttentionBackend(AttentionBackend):
    ...
    def forward_extend(
        ...
        if k is not None:
            ...
            if save_kv_cache and not is_cp_mode and not int2_deferred_save:
                cache_loc = (
        ...
        if not self.use_mla:
            ...
            if self.kv_cache_dtype_str == "int2":
                ...
                unified_k = (
                    torch.cat(unified_k_parts, dim=0)
                    if unified_k_parts
                    else k3[:0]
                )
                unified_v = (
                    torch.cat(unified_v_parts, dim=0)
                    if unified_v_parts
                    else v3[:0]
                )
                ...
                result = apply_inverse_v_rotation(
                    result, kv_pool, layer, need_v_inverse
                )
