...
class CudaGraphRunner:
    ...
    def capture_one_batch_size(
        ...
        if (
            ...
            and getattr(self.model_runner.token_to_kv_pool, "mixed_kv_enabled", None)
            is not None
    ...
    def replay_prepare(
        ...
        if (
            getattr(self.model_runner.token_to_kv_pool, "mixed_kv_enabled", None)
            is not None
            and self.model_runner.token_to_kv_pool.mixed_kv_enabled()
            and raw_bs < bs
        ):
            # The unified mixed-KV allocator reserves page 0 for padded graph
            # writes. Keep padded seq_lens at the graph fill value (1): the
            # Triton split planner assumes positive lengths across the static
            # graph batch.
            buffers.out_cache_loc[raw_num_token : bs * self.num_tokens_per_bs].fill_(
