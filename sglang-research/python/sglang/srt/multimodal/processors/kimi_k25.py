...
def normalize_and_patchify(*args, **kwargs):
    """Lazy stand-in for upstream's kernel of the same name.

    Upstream imports this at module scope from sglang.kernels.ops.mm.process,
    a package this fork does not vendor -- which made importing this module
    fail, taking the text-only path and the processor registry down with it
    over a kernel only GPU image preprocessing uses.

    Importing here keeps the module loadable and the CPU image path
    (_cpu_call) working, and turns the GPU path's failure into a sentence that
    names what is missing instead of an ImportError at interpreter start.
    """
    from sglang.srt.kernels_compat import kernels_module_available

    if not kernels_module_available("sglang.kernels.ops.mm.process"):
        raise RuntimeError(
            "Kimi-K2.5/K3 GPU image preprocessing needs "
            "sglang.kernels.ops.mm.process.normalize_and_patchify, which this "
            "fork does not vendor. Text-only requests are unaffected; for "
            "images use the CPU processor path."
        )
    from sglang.kernels.ops.mm.process import normalize_and_patchify as _impl
