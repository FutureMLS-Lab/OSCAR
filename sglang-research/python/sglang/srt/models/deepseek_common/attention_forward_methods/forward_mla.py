...
class DeepseekMLAForwardMixin:
    ...
    def forward_absorb_prepare(
        ...
        #
        # It is why the (since removed) use_expanded_mha_cache used to be pinned
        # True: not caution, a missing gate. Computed here because only prepare has hidden_states,
        ...
        if self.q_lora_rank is not None:
            ...
            # overlap qk norm
            # and not torch.compiler.is_compiling(): torch.cuda.stream is a
            # @contextmanager, so under the piecewise CUDA-graph warmup dynamo
            # hits "Graph break under GenericContextWrappingVariable" and the
            # whole server dies during init_piecewise_cuda_graphs. The block is a
            # pure overlap optimisation whose else-branch computes the same thing
            # sequentially, so skipping it while tracing costs a little
            # concurrency and keeps BOTH the prefix cache and CUDA graphs on --
            # which is the point, since Kimi-K3's MLA latent path only reaches
            # this code at all when the forward is the real absorbed-MLA one.
            if (
