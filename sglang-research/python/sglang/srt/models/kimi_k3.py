...
class KimiMLAAttention(DeepseekV2AttentionMLA):
    """Kimi MLA weights, cached as the shared latent.

    There used to be a second layout here -- the same weights expanded back
    into per-head K/V. It stored 96 x 128 x 2 = 24,576 values per token per
    layer against the latent's 512 + 64, giving up the compression the cache
    exists to provide. It is gone, along with its ``use_expanded_mha_cache``
    gate: nothing set that attribute but this class, so every branch reading it
    in the shared DeepSeek code was dead once K3 stopped setting it.
    """
...
class KimiMoE(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Same reason as forward_mla.py: the torch.cuda.stream context manager
        # cannot be traced through by dynamo during the piecewise CUDA-graph
        # warmup, and the else-branch is the same computation without the
        # overlap.
        if (
...
class KimiDeltaAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # The checkpoint stores A_log as a flat [head_dim] == 128 vector, which
        # reads like "one decay coefficient per key channel, shared by all
        # heads". Both FLA kernels disagree: the chunk gate does
        # `tl.load(A_log + i_h)` and the recurrent decode gate does
        # `A_log + i_hv`, i.e. they index it BY HEAD and want exactly
        # local_num_heads elements. Handing them the 128-wide per-channel
        # tensor made every TP rank read the same first local_num_heads entries
        # -- right by accident on rank 0, wrong on the other seven -- and the
        # value it read was a channel coefficient being used as a head one.
        # A_log drives the gate decay, so this is a silent, rank-dependent
        # corruption of exactly the quantity whose miscalibration shows up as
        # runaway generation. Shard it by head, as upstream does.
        ...
        def _a_log_weight_loader(
            ...
            w = loaded_weight
            if w.dim() == 4:  # older [1, 1, H, 1] layout
                w = w.view(w.shape[2])
            w = w.reshape(-1)
            # local_num_heads is divided by tp_size, so the rank that selects
            # the slice has to come from the same group. They are equal in
            # every configuration used here; assert rather than let a future
            # DP-attention setup silently take the wrong slice.
            assert self.attn_tp_size == self.tp_size, (
                "A_log head sharding assumes attention TP == tensor TP, got "
                f"{self.attn_tp_size} vs {self.tp_size}"
            )
            start = get_attention_tp_rank() * self.local_num_heads
            param.data.copy_(
                w.narrow(0, start, self.local_num_heads).view(param.data.shape)
            )
    ...
    def forward(
        ...
        # Both modes now hand the RAW forget gate to the backend and let the
        # kernel activate it. Decode always did; prefill used to call
        # fused_kda_gate here and pass the activated result, and that was the
        # bug. The chunk kernels read the gate cumsum with exp2, so it has to be
        # accumulated in log2 space -- chunk_kda_fwd does that by passing
        # scale=log2(e) into the cumsum. A caller that activates in natural-log
        # space and hands the result over gets exp2(g) where exp(g) was meant,
        # i.e. a decay of exp(0.693*g), too weak, and only on the prefill half
        # of the model. It compounds with length, which is why short answers
        # looked fine and long ones degenerated into verbatim repetition.
        ...
        for args in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                if expert_parts is not None:
                    ...
                    if name not in params_dict:
                        # Layers this rank does not hold under PP land here.
                        continue
                    param = params_dict[name]
                    weight_loader = param.weight_loader
                    weight_loader(
