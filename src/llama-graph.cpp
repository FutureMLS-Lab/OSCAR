...
ggml_tensor * llm_graph_context::build_attn(
    ...
    // fused mixed-precision FA: decode (1 tok/stream) and prefill (many tok) both supported
    // by the per-query kernel (causal handled via the mask). Gated by LLAMA_KV_FUSED_FA.
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    // OSCAR fused mixed-precision flash decoding (decode-only; gated by LLAMA_KV_FUSED_FA).
    // Pick the F16 masks for this layer's sub-cache (SWA vs full attention).
    ggml_tensor * lp_kq_mask_cnv = is_swa ? inp->self_kq_mask_swa_cnv : inp->self_kq_mask_cnv;
    ggml_tensor * hp_kq_mask_cnv = is_swa ? inp->hp_kq_mask_swa_cnv   : inp->hp_kq_mask_cnv;
    const bool use_fused_fa = (getenv("LLAMA_KV_FUSED_FA") != nullptr) &&
                              cparams.flash_attn && (v->nb[1] <= v->nb[2]) &&
                              hp_kq_mask_cnv && lp_kq_mask_cnv;

    if (mctx_cur->has_hp() && hp_kq_mask_f32 && use_fused_fa) {
        ggml_tensor * k_hp = mctx_cur->get_k_hp(ctx0, il);
        ggml_tensor * v_hp = mctx_cur->get_v_hp(ctx0, il);
        ...
        cur = ggml_flash_attn_ext_mixed(ctx0, q_perm, k_lp_p, v_lp_p, lp_kq_mask_cnv,
                                        k_hp_p, v_hp_p, hp_kq_mask_cnv,
                                        kq_scale, hparams.f_max_alibi_bias, 0.0f);
        cur = ggml_reshape_2d(ctx0, cur, cur->ne[0]*cur->ne[1], cur->ne[2]*cur->ne[3]);
    } else if (mctx_cur->has_hp() && hp_kq_mask_f32) {
