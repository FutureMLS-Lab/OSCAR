...
broken at batch < 16**. GLM-4.7-FP8 is 96 Q / 8 KV = 12 and lands there.
...
(`--cuda-graph-max-bs 32`), 32K budget. Measured:
...
- **Rotation quality.** The calibrated rotation was fit on 10,000 tokens/layer
  ...
  calibrated 57.91 — a tie. Refitting cannot buy 13 pp.
...
- **The partial-RoPE boundary** (M2.7 is the only model here with
  ...
  on K against 2 — a 4.5× increase — so it is a diagnostic, not a fix.
