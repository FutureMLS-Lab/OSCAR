...
def load_or_create_blocks(args) -> list[list[int]]:
    ...
    dataset = load_dataset(
        ...
        "wikitext-2-raw-v1",
        split="test",
        cache_dir=(None if args.dataset_cache is None else str(args.dataset_cache)),
    )
    ...
    blocks = [
        token_ids[i : i + args.block_size] for i in range(0, usable, args.block_size)
    ]
...
def main() -> None:
    ...
    blocks = load_or_create_blocks(args)
    if args.max_blocks:
