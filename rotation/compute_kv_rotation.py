...
def main() -> None:
    ...
    if args.method == "hadamard":
        ...
        print(
            f"Method=hadamard k_head_dim={k_head_dim} "
            f"v_head_dim={v_head_dim} layers={layer_ids}"
        )
        write_hadamard_rotation(
            output_dir, k_head_dim, v_head_dim, layer_ids
        )
        return
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    hadamards = {
        "k": build_hadamard(k_head_dim),
        "v": build_hadamard(v_head_dim),
    }
