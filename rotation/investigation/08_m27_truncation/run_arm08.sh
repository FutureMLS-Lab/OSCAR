#!/bin/bash
# One MiniMax-M2.7 GPQA arm for the INT2 truncation root-cause investigation.
#
# Derived from investigation/06_m27_recheck/run_arm.sh with the three knobs that
# investigation needs made overridable instead of hardcoded:
#
#   HP_RECENT / HP_PREFIX   mixed-KV BF16 window sizes (06 pinned 256 / 64)
#   K_ROT_FILENAME / V_ROT_FILENAME   which rotation file to load, so a
#                                     data-free Hadamard can be run as the
#                                     rotation-quality bracket
#   LLOYD_MAX               codebook choice (06 pinned 0)
#
# Everything else -- tree, weights verification, GPU squat preflight, HOME/cache
# redirection away from the full /home/charlie PVC -- is kept identical so arms
# from 06 and 08 are directly comparable.
#
