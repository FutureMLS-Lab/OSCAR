...
export TRITON_CACHE_DIR=/tmp/triton_calib5; mkdir -p $TRITON_CACHE_DIR
export CUDA_VISIBLE_DEVICES=$(ls /var/run/nvidia-container-devices 2>/dev/null | paste -sd, || echo 0,1)
cd $W/rotation/qwen3-30b-a3b
rm -rf GPQA/latest
MODEL=Qwen/Qwen3-30B-A3B TP_SIZE=1 GPU=0 MEM_FRACTION_STATIC=0.82 \
MAX_RUNNING_REQUESTS=8 MAX_QUEUED_REQUESTS=256 NUM_WORKERS=4 EXTRA_SERVER_ARGS="--context-length 4096" \
DUMP_KVCACHE_TOKENS=30000 PORT=31623 DIST_PORT=41623 \
