...
export TRITON_CACHE_DIR=/tmp/triton_v14; mkdir -p $TRITON_CACHE_DIR
...
python -m sglang.launch_server --model-path "$MODEL" --served-model-name q3 \
  ...
  --mem-fraction-static 0.70 --max-running-requests 4 --disable-cuda-graph \
  --disable-radix-cache --host 127.0.0.1 --port $PORT --trust-remote-code > "$OUT/server.log" 2>&1 &
SP=$!
for i in $(seq 1 150); do curl -s localhost:$PORT/health >/dev/null 2>&1 && break; kill -0 $SP 2>/dev/null || { echo died; tail -4 "$OUT/server.log"; exit 1; }; sleep 5; done
python3 $INV/probe_sweep.py --port $PORT --out $OUT --tag int2 || true
