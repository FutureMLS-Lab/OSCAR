...
run_arm () {
  local NAME=$1 TP=$2
  echo "=== ARM $NAME tp=$TP"
  SGLANG_ENABLE_MIXED_KV_WINDOWS=1 SGLANG_ALLOW_OVERWRITE_LONGER_CONTEXT_LEN=1   SGLANG_MIXED_KV_HP_MAX_SPLITS=8 SGLANG_MIXED_KV_PREFIX_TOKENS=64 SGLANG_MIXED_KV_RECENT_TOKENS=256   SGLANG_MIXED_KV_HP_DTYPE=bfloat16 SGLANG_MIXED_KV_SCALE_DTYPE=float32   SGLANG_OSCAR_ABSORB_V_ROTATION=0 SGLANG_OSCAR_K_CLIP_RATIO=0.96 SGLANG_OSCAR_V_CLIP_RATIO=0.92 SGLANG_LLOYD_MAX=0   SGLANG_OSCAR_K_ROTATION_PATH=/home/charlie/CoQuant/.RUD/hybridmodel-testing/work/oscar/rotation/qwen3-30b-a3b/GPQA/seq30000_prompt15_group128/rotations/k_rotation_qqt_r_h_pbr.pt   SGLANG_OSCAR_V_ROTATION_PATH=/home/charlie/CoQuant/.RUD/hybridmodel-testing/work/oscar/rotation/qwen3-30b-a3b/GPQA/seq30000_prompt15_group128/rotations/v_rotation_sst_r_h_pbr.pt   python -m sglang.launch_server --model-path "$MODEL" --served-model-name q3     --tensor-parallel-size $TP --prefill-attention-backend fa3 --decode-attention-backend triton     --kv-cache-dtype int2 --kv-cache-quant-group-size 128     --mem-fraction-static 0.85 --max-running-requests 8 --disable-cuda-graph     --disable-radix-cache --host 127.0.0.1 --port $PORT --trust-remote-code > "$OUT/server_$NAME.log" 2>&1 &
  local SP=$!
