#!/bin/bash
# Verify the Qwen3MoE absorb fix: 30B-A3B + INT2 mixed-KV + ABSORB_V=1 must generate coherently.
set -euo pipefail
...
export TRITON_CACHE_DIR=/tmp/triton_absorbfix; mkdir -p $TRITON_CACHE_DIR
...
SGLANG_OSCAR_K_ROTATION_PATH=$INV/rotations/k_rotation_hadamard.pt \
SGLANG_OSCAR_V_ROTATION_PATH=$INV/rotations/v_rotation_hadamard.pt \
...
python -m sglang.launch_server --model-path "$MODEL" --served-model-name q3 \
  ...
  --disable-radix-cache --host 127.0.0.1 --port $PORT --trust-remote-code > "$OUT/server.log" 2>&1 &
SP=$!
grep -m1 "Absorbed Oscar V-rotation\|folded" "$OUT/server.log" 2>/dev/null || true
for i in $(seq 1 180); do curl -s localhost:$PORT/health >/dev/null 2>&1 && break; kill -0 $SP 2>/dev/null || { echo "server died"; tail -8 "$OUT/server.log"; exit 1; }; sleep 5; done
grep -i "absorb" "$OUT/server.log" | head -2
python3 "$RADIX/probe_client.py" --port $PORT --arm absorbfix --out "$OUT"
kill -TERM $SP 2>/dev/null || true; sleep 3; kill -KILL $SP 2>/dev/null || true; pkill -KILL -P $SP 2>/dev/null || true
echo VERIFY_DONE
