...
def compare(name, ref, got):
    ...
    exact = int((diff == 0).sum()), diff.numel()
    print(f"  {name:<34} max|d|={diff.max().item():.3e}  relL2={rel:.3e}  "
          f"bit-exact {exact[0]}/{exact[1]}")
...
def run_case(name, x, lloyd):
    ...
    got = dequantize(codes, params, x.shape, G, torch.float32)
...
def main():
    ...
    for lloyd in (False, True):
        ...
        for cname, x in cases.items():
            m, _ = run_case(f"[{tag}] {cname}", x, lloyd)
            worst = max(worst, m)
    ...
    if a.ckv and os.path.exists(a.ckv):
        ...
        for lloyd in (False, True):
            m, _ = run_case(f"[{'lloyd' if lloyd else 'uniform'}] real c_kv", c, lloyd)
            worst = max(worst, m)
        ...
        if a.rot:
            ...
            if os.path.exists(rp):
                ...
                for lloyd in (False, True):
                    ...
                    got = dequantize(codes, params, rot.shape, G, torch.float32) @ R.T
                    m, _ = compare(f"[{'lloyd' if lloyd else 'uniform'}] rotate->q->unrotate",
                                   ref, got)
                    worst = max(worst, m)
    ...
    print(f"\nworst max|diff| across all cases: {worst:.3e}")
    ok = worst < 1e-4
    print("VERDICT:", "MATCHES the simulation" if ok else "DIVERGES from the simulation")
